...
import { PostDeviceMutation } from "../hooks/PostDeviceMutation";
...
const DropFileInput = () =>{
    ...
    const {data,isSuccess,mutate,isPending} = PostDeviceMutation();
    ...
    const onFileUpload = () =>{
        if(excelFile.name !== undefined && wrapperRef.current){
            mutate({
                file:excelFile,
                updateBy:userDetail.data.result.divisionId
            });
            setExcelFile("");
            wrapperRef.current.value = ''
        }
    ...
    return(
        <>
            <div className="h-full px-8 py-8 bg-white border-2 border-gray-300 rounded-md hover:border-gray-700">
                <h1 className="mb-8 text-xl font-semibold text-center">Upload Update Device Excel File</h1>
...

            {
                isOpen && 
                        <ChakraUiModal
                            isOpen={isOpen}
                            onClose={onClose}
                            modalHeader={`Device Info Excel Update`}
                        >
                            <div>
                                {
                                    isSuccess ?
